    ...
    value: function values(contract, statement, _values) {
      ...
      return global.describe(statement, function () {
        return keys.forEach(function (key) {
          ...
          return _this2.assert(method, 'value \'' + key + '\' is correct', [[[], expected]], transform);
        });
      });
    }
  }, {
    ...
    value: function assert() {
      ...
      return (0, _methodTester2.default)('assert', opts, function (_ref) {
        ...
        return promise.then(function (outs) {
          ...
          console.log('expectedddd', outputs);
          expected.forEach(function (expectedOutput, i) {
            console.log('eo', expectedOutput);
            // tranform output
            console.log('trans', transformers[i]);
            var transformedOutput = transformers[i] ? transformers[i](outputs[i], params) : outputs[i];
            console.log('to', transformedOutput, outputs[i], outputs);
